...
authorization_list = [config.get('bots', 'bots_1')]
channel_id = config.get('channel', 'channel_id')
bot_gap = config.getint('time', 'bot_gap')
...
def chat():
    ...
    for authorization in authorization_list:
        ...
        try:
            ...
            result= res.json()
            print('已发送第'+str(talk_counter + 1 )+'句话，内容:',result['content'])

            time.sleep(random.randrange(bot_gap,bot_gap+5))
        except:
...
class App:
 def __init__(self, root):
    ...
    root.title("Dsicord Talker V1.0")
    ...
    label_1 = tk.Label(root, text="语料读取完毕,共" + str(len(talk_list)) + "条 || bot发言间隔" + str(bot_gap) + "秒", bg="CornflowerBlue", font=("微软雅黑", 12), )
 ...
 def talk_loop(self):
     ...
     while talk_pause == 0:
        try:
            ...
            #sleeptime = random.randrange(60, 180) # sleep 1-3 mins
...
explanation = """discord.gg/cryptochasers                 
...
"""
tk.Label(root,compound=tk.CENTER,text=explanation, fg="DarkGreen",font=("微软雅黑", 12,"bold"),image=logo).pack(side="right")
